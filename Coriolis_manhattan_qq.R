...
mapfile<-"/data/home/btx586/Coriolis/chr.bp.rsid"
...
write.table(merge1,paste(datafile,".chr.bp",sep=""),row.names=F,col.names=T,quote=F,sep="\t")
...
for(j in 1:22){
	if(j==22){
		plot(frame=F,data$plot.pos[data$CHR==j & data$FE_P<0.05],-log10(data$FE_P[data$CHR==j & data$FE_P<0.05]),ylim=c(2,plot.max),col=colours[j],ylab="-log10(p-value)",xlim=c(0,max(data$plot.pos)),pch=20,xaxt='n',xlab="Chromosomes",cex=1)
		par(new=T)
	}
	else { 
		plot(frame=F,data$plot.pos[data$CHR==j & data$FE_P<0.05],-log10(data$FE_P[data$CHR==j & data$FE_P<0.05]),ylim=c(2,plot.max),col=colours[j],ylab="",xlim=c(0,max(data$plot.pos)),pch=20,xaxt='n',xlab="",cex=1,yaxt='n')
		par(new=T)
	}
}
