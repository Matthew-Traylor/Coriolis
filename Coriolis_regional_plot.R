...
png(paste(args[4],".png",sep=""),2*2**10,2*2**10,pointsize=10,res=300)

par(mai=c(0,0.75,0.5,0.75))
par(mfrow=c(2,1))
par(xpd=NA)->xpd

ld<-read.table(args[6],header=T,strings=F,colClasses=c("numeric","numeric","character","numeric","numeric","character","numeric"))
SNP.name<-ld[1,3]
names(ld)[6]<-"SNP"
data<-read.table(args[4],header=F,strings=F)
names(data)[1]<-"SNP"
...
ymax<-max(-log10(data$V33)+1.5)
topsnp<-data[which(data$SNP==SNP.name),1]
topp<-data[which(data$SNP==SNP.name),10]
raf<-data[which(data$SNP==SNP.name),6]
OR<-formatC(exp(data[which(data$SNP==SNP.name),8]),2,format='f')
#lci<-formatC(exp(data[which(data$SNP==SNP.name),8]-1.96*data[which(data$SNP==SNP.name),9]),2,format='f')
#uci<-formatC(exp(data[which(data$SNP==SNP.name),8]+1.96*data[which(data$SNP==SNP.name),9]),2,format='f')
data_nor2<-subset(data,!(R2>0.2))
data_r2<-subset(data,(R2>0.2))
CHR<-args[5]
CM.Mb<-read.table(paste("/data/home/btx586/Coriolis/resources/genetic_map_chr",CHR,"_combined_b37.txt",sep=""),header=F,skip=1)
names(CM.Mb)<-c("V3","CM.Mb","CM")

#generate plot in two steps; 1/ SNPs not in LD with top SNP, 2/ SNPs in LD with top SNP
plot(frame=F,data_nor2$V3,-log10(data_nor2$V33),bg=ifelse(data_nor2$R2>0.8,cols[1],ifelse(data_nor2$R2>0.6,cols[2],ifelse(data_nor2$R2>0.4,cols[3],ifelse(data_nor2$R2>0.2,cols[4],"grey")))),pch=21, col=ifelse(data_nor2$R2>0.2,"black","grey"),ylab="",xlab="",ylim=c(0,ymax),xlim=c(BP_min,BP_max),xaxt='n',yaxt='n')
par(new=T)
plot(frame=F,data_r2$V3,-log10(data_r2$V33),
	...
	ylab="-log10(p-value)",xlab=paste("Genomic position on Chromosome ",args[5],sep=""),
...
text((BP_min+BP_max)/2,ymax-0.5,paste(topsnp,", p=",formatC(topp,2)))#,"\nRAF=",formatC(raf,2),", OR(95% CI)=",OR,"(",lci,"-",uci,")",sep=""))

#add recombination rate
par(xpd)
lines(CM.Mb$V3,ymax*(CM.Mb$CM.Mb/100),col="tomato3")
axis(4,col="tomato3",col.axis="tomato3",at=c(0,ymax/4,ymax/2,3*ymax/4,ymax),labels=c(0,25,50,75,100))
mtext("Recombiation Rate (CM/Mb)",4,col="tomato3",cex=0.9,line=3)
par(new=T)
plot(frame=F,data_nor2$V3,-log10(data_nor2$V33),bg=ifelse(data_nor2$R2>0.8,cols[1],ifelse(data_nor2$R2>0.6,cols[2],ifelse(data_nor2$R2>0.4,cols[3],ifelse(data_nor2$R2>0.2,cols[4],"grey")))),pch=21, col=ifelse(data_nor2$R2>0.2,"black","grey"),ylab="",xlab="",ylim=c(0,ymax),xlim=c(BP_min,BP_max),xaxt='n',yaxt='n')
par(new=T)
plot(frame=F,data_r2$V3,-log10(data_r2$V33),
...
genemap<-read.table("/data/home/btx586/Coriolis/resources/glist-hg19",header=F,colClasses=c("character","integer","integer","character"))
